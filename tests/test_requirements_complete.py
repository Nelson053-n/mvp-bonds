...
# fpdf — запасной генератор PDF на случай отсутствия reportlab. Кириллицу он
# не умеет (ASCII-only), поэтому как основной путь не годится; reportlab же
# теперь объявлен в requirements, и до этой ветки дело не доходит.
_OPTIONAL = {"fpdf"}
